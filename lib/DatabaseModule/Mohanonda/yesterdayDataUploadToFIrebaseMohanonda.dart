...
class YesterdayDataUploadToFirebaseMohanonda {
  upload(TodayReportMohanondaDataModule yesterdayData) async {
    final databaseReference = FirebaseDatabase.instance.reference();
    var date = DateFormat("dd-MM-yyyy")
        .format(DateTime.now().subtract(Duration(days: 1)));
    var yesterdayVehicle = yesterdayData.yesterdayVehicleReportList;
    databaseReference.child('Mohanonda').child(date.toString()).update({
      'date': date.toString(),
  ...
  uploadVIP(TodayVipPassReportMohanondaDataModule yesterdayData) async {
    final databaseReference = FirebaseDatabase.instance.reference();
    var date = DateFormat("dd-MM-yyyy")
        .format(DateTime.now().subtract(Duration(days: 1)));
